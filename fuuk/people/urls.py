...
urlpatterns = patterns('',
    ...
    url(r'^thesis/id=(?P<pk>\d+)/$', DetailView.as_view(model=Thesis)),
    url(r'^courses/$', ListView.as_view(model=Course, template_name='courses.html'), name="courses"),
    url(r'^downloads/$', ListView.as_view(queryset=Course.objects.exclude(attachment__isnull=True),
                                          template_name='downloads.html'), name="downloads"),
...
human_patterns = patterns('',
    ...
    url(r'^(?P<slug>\w+)/papers/(?P<first>first)/$', PersonArticles.as_view(), name="person_articles"),
